"""Dark Tkinter GUI for the HalfCheetah PPO/SAC/TD3 workbench."""
...
from halfcheetah_logic import (
    ...
    SOLVED_RETURN, TUPLE_FIELDS, EpisodeMetric, EvaluationResult, HalfCheetahConfig,
    HalfCheetahWorkbench, action_readout, config_differences, default_config,
    observation_readout, reward_readout,
)
from halfcheetah_render import HalfCheetahRenderer, RendererUnavailable
...
#: (`env.metadata["render_fps"] == 20`, also 1/dt mit dt = 0,05 s). Eine volle
#: Episode dauert damit 50 Sekunden – zum Zuschauen ist eine höhere Rate hier
#: die Regel.
RENDER_FPS = 20
...
PARAMETER_GROUPS: dict[str, tuple[tuple[str, str, tuple[str, ...]], ...]] = {
    "PPO": (
        ...
        ("left", "Rollout und geclipptes Ziel",
         ...
         # Zoo-Profil schaltet sie für HalfCheetah ab. Sie steht hier statt in der
...
class SlotRuntime:
    ...
    def __init__(self, config: HalfCheetahConfig, best_base: Path) -> None:
        self.workbench = HalfCheetahWorkbench(config)
        self.comparison: Optional[HalfCheetahWorkbench] = None
    ...
    def reset_all(self, config: HalfCheetahConfig) -> None:
...
class HalfCheetahGUI:
    ...
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.renderers: list[Optional[HalfCheetahRenderer]] = [None] * MAX_SLOTS
        ...
        self.checkpoints = tempfile.TemporaryDirectory(prefix="halfcheetah-")
    ...
    @staticmethod
    def initial_config(slot: int) -> HalfCheetahConfig:
    ...
    def _variables(self) -> None:
        ...
        for slot in range(MAX_SLOTS):
            ...
            for field in fields(HalfCheetahConfig):
    ...
    def _fill_values(self, slot: int, config: HalfCheetahConfig) -> None:
    ...
    def _window(self) -> None:
        ...
        self.root.title("HalfCheetah Workbench")
    ...
    def _layout(self) -> None:
        ...
        ttk.Label(header, text="HalfCheetah Workbench",
    ...
    def _panel_configs(self) -> list[HalfCheetahConfig]:
    ...
    def _config(self, slot: int) -> HalfCheetahConfig:
        data: dict[str, Any] = {}
        for name, variable in self.values[slot].items():
            if name in BOOLEAN_FIELDS:
        ...
        config = HalfCheetahConfig(algorithm=self.algorithm_vars[slot].get(), **data)
        try:
            config.validate()
        except ValueError as error:
    ...
    def start_training(self) -> None:
        ...
        if runtime.workbench.model is not None \
            ...
            runtime.workbench = HalfCheetahWorkbench(config)
    ...
    def start_comparison(self) -> None:
        ...
        for slot, config in enumerate(configs):
            ...
            if runtime.comparison is None:
                runtime.comparison = HalfCheetahWorkbench(config)
            else:
    ...
    def restore_best(self) -> None:
        ...
        if runtime.best is None:
            return
        try:
            restored = HalfCheetahWorkbench.load(runtime.best_base, runtime.algorithm)
        except Exception as error:
    ...
    def _series_label(self, slot: int, configs: Optional[list[HalfCheetahConfig]] = None,
    ...
    def _statistics(cls, history: list[EpisodeMetric], steps: int, budget: int) -> tuple[Any, ...]:
        """Eine Zeile für die Episodenlänge gibt es bewusst nicht: Sie wäre
        immer 1000, weil das Environment nie terminiert."""
        if not history:
            return (0, thousands(steps), thousands(budget), *(["—"] * 7))
        ...
        return (
            ...
            german(float(np.mean([item.ctrl_cost for item in history])), 1),
            f"{np.mean([item.forward for item in history]):.1%}".replace(".", ","),
            f"{np.mean([item.solved for item in history]):.1%}".replace(".", ","),
        )

    STAT_LABELS = ("Episoden", "Schritte", "Budget", "Ø Return", "Beste Episode",
                   "Ø Tempo m/s", "Ø Strecke m", "Ø Steuerkosten",
                   "Vorwärtsquote", "Gelöst-Quote")
    ...
    def _evaluation_footer(self, prefix: str, result: EvaluationResult) -> str:
        return (f"{prefix}: Ø {german(result.mean_reward)} ± {german(result.reward_std)} | "
                f"vorwärts {result.forward_rate:.0%} | gelöst {result.solved_rate:.0%} | "
                f"v̄ₓ {german(result.mean_speed, 2)} m/s | "
                f"Strecke {german(result.mean_distance, 1)} m | "
                f"Steuerkosten {german(result.mean_ctrl_cost, 1)}")
    ...
    def _training_summary(self) -> None:
        ...
        footer = ["Höhere Returns sind besser; gelöst ab "
                  ...
                  f"Das Environment terminiert nie: Jede Episode dauert exakt "
                  f"{MAX_EPISODE_STEPS} Schritte. Vorwärts = mittleres vₓ > 0."]
    ...
    def _difference_lines(self, configs: list[HalfCheetahConfig]) -> list[str]:
    ...
    def _export_base_name(self) -> str:
        ...
        return f"halfcheetah_{slug}_{self._export_stamp}"
    ...
    def _renderer(self, slot: int) -> HalfCheetahRenderer:
        ...
        if self.renderers[slot] is None:
            self.renderers[slot] = HalfCheetahRenderer(seed=self.slots[slot].workbench.config.seed)
    ...
    def _animation_workbench(self, slot: int) -> HalfCheetahWorkbench:
    ...
    def _hover_lines(self, slot: int) -> str:
        ...
        # Jedes Gelenk hat seine eigene Übersetzung – ein gemeinsamer Faktor
        # wie beim Hopper wäre hier falsch.
        lines = ["Action (Moment = aᵢ · gear des Gelenks)"]
        ...
        lines += [
            ...
            "  Winkel      hinten " + " ".join(f"{value:+.2f}" for value in angles[:3]),
            "              vorn   " + " ".join(f"{value:+.2f}" for value in angles[3:]),
            f"  Tempo       vₓ {values['vx']:+.3f}  v_z {values['vz']:+.3f}  "
            f"ω {values['torso_angular_velocity']:+.3f}",
            "  Gelenktempo hinten " + " ".join(f"{value:+.2f}" for value in speeds[:3]),
            "              vorn   " + " ".join(f"{value:+.2f}" for value in speeds[3:]),
            "  Geschwindigkeiten sind unbeschränkt.",
            "",
            "Reward des letzten Schritts",
            f"  vorwärts {rewards['forward']:+.3f}   Steuerkosten {rewards['ctrl']:+.4f}",
    ...
    def _episode_outcome_text(terminated: bool) -> str:
        """`HalfCheetah-v5` besitzt keinen terminalen Zustand: `terminated` ist
    ...
    def instructions(self) -> None:
        messagebox.showinfo(
            ...
            "Environment\n"
            "HalfCheetah-v5 (MuJoCo): a₀ bis a₅ sind Drehmomente für sechs Gelenke –\n"
            "hinten und vorn je Hüfte, Knie und Fuß. Jeder Motor hat eine EIGENE\n"
            "Übersetzung (120, 90, 60, 120, 60, 30), derselbe Actionwert bedeutet also\n"
            "je Gelenk ein anderes Moment. Der Reward ist 1,0·vₓ - 0,1·Σaᵢ²; einen\n"
            "Überlebensbonus gibt es nicht, und die Steuerkosten sind hundertmal so\n"
            "hoch wie beim Hopper – wildes Zappeln kostet sichtbar Return.\n"
            "Das Environment TERMINIERT NIE: Der Roboter kann auf dem Rücken landen\n"
            "oder rückwärts rudern, die Episode läuft weiter. Beendet wird nur durch\n"
            f"das Zeitlimit nach {MAX_EPISODE_STEPS} Schritten, jede Episode ist also\n"
            "gleich lang. Deshalb gibt es hier keine Sturz- oder Durchhaltequote,\n"
            "sondern die Vorwärtsquote (mittleres vₓ > 0). Rückwärtslaufen liefert\n"
            f"negative Returns. Gelöst ab Return {german(SOLVED_RETURN, 0)}.\n\n"
            "Methoden\n"
            ...
            "die Gelöst-Marke; die Y-Achse folgt den Messwerten.\n"
            ...
            "eine zu hohe Lernrate oder bei PPO fehlende Normalisierung. Mit dem vollen\n"
            f"Budget ist die {german(SOLVED_RETURN, 0)} laut Zoo-Benchmark aber erreichbar:\n"
            "PPO rund 5.819, SAC rund 9.535, TD3 rund 9.656 – der große Abstand zeigt\n"
            "den Vorteil der Off-Policy-Verfahren bei gleichem Schrittbudget.",
            parent=self.root,
        )
