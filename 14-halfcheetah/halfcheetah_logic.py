"""SB3 learning logic for the HalfCheetah workbench."""
...
#: Offizieller Gymnasium-`reward_threshold` von `HalfCheetah-v5`.
SOLVED_RETURN = 4800.0
#: Zeitlimit des `TimeLimit`-Wrappers. Weil das Environment nie terminiert, ist
#: das zugleich die Länge **jeder** Episode.
MAX_EPISODE_STEPS = 1000
#: Box(-1, 1, (6,)): Drehmomente für sechs Gelenke.
ACTION_DIM = 6
JOINT_NAMES = ("Hüfte hinten", "Knie hinten", "Fuß hinten",
               "Hüfte vorn", "Knie vorn", "Fuß vorn")
#: Übersetzung je Motor aus `half_cheetah.xml`. Anders als beim Hopper hat
#: **jedes** Gelenk eine eigene: Derselbe Actionwert bedeutet je Gelenk ein
#: anderes Moment. Ein Test vergleicht die Tabelle mit `model.actuator_gear`.
GEARS = (120.0, 90.0, 60.0, 120.0, 60.0, 30.0)
#: 17 Beobachtungswerte; die x-Position ist bewusst nicht enthalten.
OBSERVATION_DIM = 17
#: Gewichte des Rewards. `ctrl_cost_weight` ist hundertmal so groß wie beim
#: Hopper: Wildes Zappeln kostet hier sichtbar Return.
FORWARD_REWARD_WEIGHT = 1.0
CTRL_COST_WEIGHT = 0.1
#: Simulationsschritt: `frame_skip = 5` mal `timestep = 0.01`.
STEP_DURATION = 0.05
...
#: Getunte `HalfCheetah-v4`-Profile des RL Baselines3 Zoo (Stand geprüft am
#: 20.08.2026). Werte, die dort nicht auftauchen, bleiben auf den
#: Voreinstellungen von Stable-Baselines3 (siehe Dataclass-Defaults).
DEFAULT_PROFILES: dict[str, dict[str, Any]] = {
    # Der Zoo führt für HalfCheetah einen vollständig getunten PPO-Block – und
    # zwar mit `n_envs: 1`. Der Rollout von 512 Schritten je Update entspricht
    # hier also exakt dem Profil.
    "PPO": {
        "total_timesteps": DEFAULT_TOTAL_TIMESTEPS, "learning_rate": 2.0633e-5,
        "gamma": 0.98, "n_steps": 512, "batch_size": 64, "n_epochs": 20,
        "gae_lambda": 0.92, "clip_range": 0.1, "ent_coef": 0.000401762,
        "vf_coef": 0.58096, "max_grad_norm": 0.8,
        "log_std_init": -2.0, "ortho_init": False,
        "actor_arch": DEFAULT_NET_ARCH, "critic_arch": DEFAULT_NET_ARCH, "activation": "ReLU",
        # Das Zoo-Profil verlangt `normalize: true`: Die 17 Werte sind sehr
        # unterschiedlich skaliert, die Geschwindigkeiten ungeclippt und der
        # Return wird fünfstellig.
...
def make_halfcheetah_env(render_mode: Optional[str] = "rgb_array") -> gymnasium.Env:
    """Offizielles Environment ohne jede Änderung an Physik und Reward.

    Sämtliche Parameter von `HalfCheetahEnv` – `forward_reward_weight`,
...
class HalfCheetahConfig:
    ...
    def policy_kwargs(self) -> dict[str, Any]:
        ...
        if self.algorithm in ("PPO", "SAC"):
            ...
            # PPO-Zoo-Profil setzt sie für HalfCheetah auf -2.
            kwargs["log_std_init"] = self.log_std_init
        if self.algorithm == "PPO":
            # Initialisierung gehört laut Workbench in die UI; das Zoo-Profil
            # schaltet die orthogonale Initialisierung für HalfCheetah ab.
    ...
    @classmethod
    def from_dict(cls, values: dict[str, Any]) -> "HalfCheetahConfig":
        data = dict(values)
...
def default_config(algorithm: str) -> HalfCheetahConfig:
    ...
    return HalfCheetahConfig(algorithm=algorithm, **DEFAULT_PROFILES[algorithm])
...
def config_differences(configs: Sequence[HalfCheetahConfig]) -> list[tuple[str, list[str]]]:
...
class EpisodeMetric:
    ...
    solved: bool
    timesteps: int
    #: Mittleres `vₓ` der Episode in m/s; zugleich das Vorzeichenkriterium für
    #: `forward`. Die Strecke ist `mean_speed · length · STEP_DURATION`.
    mean_speed: float = 0.0
    #: Zurückgelegte Strecke am Episodenende aus `info["x_position"]`.
    distance: float = 0.0
    #: Summe der Steuerkosten der Episode, negativ wie `reward_ctrl` in `info`.
    ctrl_cost: float = 0.0
...
class EvaluationResult:
    ...
    mean_reward: float
    reward_std: float
    #: Konstant `MAX_EPISODE_STEPS`, weil das Environment nie terminiert. Nicht
    #: als Summary-Zeile geführt, aber für den Test vorhanden.
    mean_length: float
    forward_rate: float
    backward_rate: float
    solved_rate: float
...
def episode_outcome(episode_return: float, mean_speed: float) -> tuple[bool, bool]:
    """Ausgang einer beendeten Episode als (vorwärts, gelöst).

    `HalfCheetah-v5` besitzt **keinen terminalen Zustand**: Es liefert immer
    `terminated=False`, und jede Episode endet nach `MAX_EPISODE_STEPS` durch
    den `TimeLimit`-Wrapper. Ein Sturz existiert nicht – der Roboter kann auf
    dem Rücken landen oder rückwärts rudern, die Episode läuft weiter. Deshalb
    gibt es hier weder Sturz- noch Durchhaltequote; unterschieden wird nur, ob
    der Roboter netto vorangekommen ist.
    """
    forward = mean_speed > 0.0
...
def observation_readout(observation: np.ndarray) -> dict[str, Any]:
    ...
    Echte physikalische Größen in Meter, Radiant und pro Sekunde. Anders als
    beim Hopper sind die Geschwindigkeiten **nicht geclippt**: `HalfCheetah-v5`
    gibt `qvel` unverändert aus. Die x-Position steht nicht in der Observation.
    """
...
def action_readout(action: np.ndarray) -> dict[str, Any]:
    ...
    Das Vorzeichen bestimmt die Drehrichtung, der Betrag mal der **gelenkeigenen**
    Übersetzung das anliegende Moment in Newtonmetern. Ein gemeinsamer Faktor
    wie beim Hopper wäre hier falsch: Die sechs Motoren haben Übersetzungen
    zwischen 30 und 120. Ein Wert von 0 bedeutet „kein Moment", nicht „Gelenk
    hält die Position". Werte außerhalb [-1, 1] clippt das Environment
    (`ctrlrange="-1 1"`).
    """
    values = np.clip(np.asarray(action, dtype=float).reshape(-1), -1.0, 1.0)
    joints = []
    for name, gear, value in zip(JOINT_NAMES, GEARS, values):
...
def control_cost(action: np.ndarray) -> float:
    """`ctrl_cost = 0,1 · Σ aᵢ²` – höchstens 0,6 je Schritt."""
    values = np.clip(np.asarray(action, dtype=float).reshape(-1), -1.0, 1.0)
    return float(CTRL_COST_WEIGHT * np.sum(np.square(values)))


def reward_readout(info: dict[str, Any]) -> dict[str, float]:
    """Die beiden Reward-Anteile direkt aus `info` übernehmen.

    `HalfCheetah-v5` liefert `reward_forward` und `reward_ctrl` (bereits
    negativ); einen Überlebensbonus gibt es nicht. Die Werte werden übernommen
...
class HalfCheetahCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if done:
            ...
            forward, solved = episode_outcome(total, mean_speed)
            metric = EpisodeMetric(
                self.episode_offset + len(self.metrics) + 1, total, length,
                forward, solved, int(self.num_timesteps),
                mean_speed, self.distance, self.ctrl_sum,
            )
            ...
            self.speed_sum, self.ctrl_sum, self.distance = 0.0, 0.0, 0.0
...
class HalfCheetahWorkbench:
    ...
    def _make_training_env(self) -> Any:
        ...
        """
        env: Any = DummyVecEnv([lambda: Monitor(make_halfcheetah_env(render_mode=None))])
    ...
    def evaluate(self, episodes: int, seed: int = 0) -> EvaluationResult:
        ...
        bleiben damit in Originaleinheiten und sind mit der Schwelle von 4800
        vergleichbar.
        """
        if self.model is None:
            raise RuntimeError("Vor der Evaluation muss ein Modell trainiert sein.")
        env = make_halfcheetah_env(render_mode=None)
        rewards, lengths, speeds, distances, costs = [], [], [], [], []
        forwards = solutions = 0
        try:
            for index in range(episodes):
                observation, _ = env.reset(seed=seed + index)
                total, length = 0.0, 0
                terminated = truncated = False
                speed_sum = cost_sum = distance = 0.0
                ...
                forwards += int(forward)
                solutions += int(solved)
        finally:
            env.close()
        forward_rate = forwards / episodes
        return EvaluationResult(
            episodes, mean(rewards), pstdev(rewards), mean(lengths),
            forward_rate, 1.0 - forward_rate, solutions / episodes,
            mean(speeds), mean(distances), mean(costs),
        )
    ...
    def load(
        ...
        workbench = cls(HalfCheetahConfig.from_dict(metadata["config"]))
