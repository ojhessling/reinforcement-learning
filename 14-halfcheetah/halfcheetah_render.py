"""Isolated official Gymnasium/MuJoCo renderer for macOS-safe Tk integration."""
...
def _worker(connection: Connection, seed: Optional[int]) -> None:
    ...
    try:
        ...
        from halfcheetah_logic import make_halfcheetah_env
...
class HalfCheetahRenderer:
    ...
    def _await_ready(self, timeout: float) -> bool:
        if self.ready:
            return True
        if self.closed:
            raise RendererUnavailable("Der isolierte HalfCheetah-Renderer ist bereits beendet.")
    ...
    def _exchange(self, message: tuple[str, object]):
        ...
        if self.closed or not self.process.is_alive():
            raise RendererUnavailable("Der isolierte HalfCheetah-Renderer ist nicht verfügbar.")
