"""Dark Tkinter GUI for the Walker2d TD3/SAC/CMA-ES workbench."""
...
from walker2d_logic import (
    ALGORITHMS, BOOLEAN_FIELDS, CHOICE_FIELDS, DEFAULT_EVALUATION_EPISODES, GEAR,
    TEXT_FIELDS,
    DEFAULT_EVALUATION_INTERVAL, DEFAULT_TOTAL_TIMESTEPS, INTEGER_FIELDS, MAX_EPISODE_STEPS,
    OPTIONAL_FLOAT_FIELDS,
    TARGET_RETURN, TUPLE_FIELDS, EpisodeMetric, EvaluationResult, Walker2dConfig,
    action_readout, config_differences, default_config, load_workbench,
    make_workbench, observation_readout, reward_readout,
)
from walker2d_render import Walker2dRenderer, RendererUnavailable
...
#: (`env.metadata["render_fps"] == 125`, also 1/dt mit dt = 0,008 s). Eine volle
...
MAX_SLOTS = 4
SLOT_COUNTS = (2, 3, 4)
#: Startbelegung: die drei Verfahren des Projekts nebeneinander, jedes genau
#: einmal. Ein vierter Slot würde zwangsläufig eines doppeln.
DEFAULT_SLOT_COUNT = 3
DEFAULT_SLOT_ALGORITHMS = ("TD3", "SAC", "CMA-ES", "SAC")
#: Abweichungen vom Profil, die **nur** die Startbelegung setzt. In der
#: Standardbelegung kommt kein Algorithmus doppelt vor; erst ein vierter Slot
#: doppelt SAC und bekommt deshalb eine abweichende Lernrate.
...
SHORT_LABELS = {
    ...
    "sigma0": "Sigma σ₀", "popsize": "Popul. λ",
    "episodes_per_candidate": "Ep./Kand.", "diagonal": "Diagonal",
...
#: CMA-ES kennt weder Lernrate noch Batch-Größe noch Diskontfaktor.
_CMA_TRAINING = ("total_timesteps", "seed")
_CMA_SEARCH = ("sigma0", "popsize", "episodes_per_candidate", "diagonal")
#: Rangbasiert: Eine Reward-Normalisierung wäre wirkungslos, das Feld entfällt.
_CMA_NORMALIZATION = ("clip_obs", "normalize_obs")
_CMA_POLICY = ("actor_arch", "activation")
...
#: Die 17 Werte sind sehr unterschiedlich skaliert; die Gruppe erscheint
#: deshalb in jedem Tab – bei CMA-ES ohne das Feld für Rewards.
...
PARAMETER_GROUPS: dict[str, tuple[tuple[str, str, tuple[str, ...]], ...]] = {
    # Gradientenfrei: kein Replay Buffer, keine Lernrate, kein Optimizer, kein
    # Diskontfaktor, kein Action Noise – und keine Reward-Normalisierung.
    "CMA-ES": (
        ("left", "Training", _CMA_TRAINING),
        ("left", "Suchverteilung", _CMA_SEARCH),
        ("right", "Policy", _CMA_POLICY),
        ("right", "Normalisierung", _CMA_NORMALIZATION),
    ),
    "SAC": (
        _TRAINING_GROUP,
...
class SlotRuntime:
    ...
    def __init__(self, config: Walker2dConfig, best_base: Path) -> None:
        self.workbench = make_workbench(config)
    ...
    def reset_all(self, config: Walker2dConfig) -> None:
...
class Walker2dGUI:
    ...
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.renderers: list[Optional[Walker2dRenderer]] = [None] * MAX_SLOTS
        ...
        #: Lernstand, den die Animation je Slot spielt – gezogen zu Beginn
        #: jeder sichtbaren Episode, damit die GUI nie in ein lernendes Netz
        #: greift.
        self.animation_snapshot: list[Any] = [None] * MAX_SLOTS
        ...
        self.checkpoints = tempfile.TemporaryDirectory(prefix="walker2d-")
    ...
    @staticmethod
    def initial_config(slot: int) -> Walker2dConfig:
    ...
    def _variables(self) -> None:
        ...
        for slot in range(MAX_SLOTS):
            ...
            for field in fields(Walker2dConfig):
    ...
    def _fill_values(self, slot: int, config: Walker2dConfig) -> None:
    ...
    def _window(self) -> None:
        ...
        self.root.title("Walker2d Workbench")
    ...
    def _layout(self) -> None:
        ...
        ttk.Label(header, text="Walker2d Workbench",
                  font=("TkDefaultFont", 18, "bold")).pack(side="left")
        ttk.Label(header, text="TD3, SAC und CMA-ES – gradientenbasiert gegen "
                               "gradientenfrei").pack(side="left", padx=16)
    ...
    def _panel_configs(self) -> list[Walker2dConfig]:
    ...
    def _config(self, slot: int) -> Walker2dConfig:
        data: dict[str, Any] = {}
        for name, variable in self.values[slot].items():
            if name in BOOLEAN_FIELDS:
            ...
            try:
                if name in TUPLE_FIELDS:
                    ...
                    data[name] = int(text)
                elif name in CHOICE_FIELDS or name in TEXT_FIELDS:
                    data[name] = text
                else:
        ...
        config = Walker2dConfig(algorithm=self.algorithm_vars[slot].get(), **data)
        try:
            config.validate()
        except ValueError as error:
    ...
    def start_training(self) -> None:
        ...
        if runtime.workbench.model is not None \
            ...
            runtime.workbench = make_workbench(config)
    ...
    def start_comparison(self) -> None:
        ...
        for slot, config in enumerate(configs):
            ...
            if runtime.comparison is None:
                runtime.comparison = make_workbench(config)
            else:
    ...
    def restore_best(self) -> None:
        ...
        if runtime.best is None:
            return
        try:
            restored = load_workbench(runtime.best_base, runtime.algorithm)
        except Exception as error:
    ...
    def _limit_to_data(axes: Any, values: list[float]) -> None:
        ...
        Die Gelöst-Marke von 4800 liegt anfangs weit über den Messwerten – ein
        untrainierter Agent läuft rückwärts und landet im Negativen. Würde die
        Achse sie erzwingen, wären alle Kurven zusammengedrückt; die Legende
        nennt den Wert stattdessen ausdrücklich.
        """
        if not values:
            return
    ...
    def _reference_line(self, axes: Any) -> None:
        axes.axhline(TARGET_RETURN, color=REFERENCE_COLOR, linestyle=REFERENCE_STYLE,
                     ...
                     label=f"Zielmarke {german(TARGET_RETURN, 0)}")

    def _series_label(self, slot: int, configs: Optional[list[Walker2dConfig]] = None,
                      prefix: Optional[str] = None) -> str:
        """`V1 – TD3`; teilen sich mehrere Slots einen Algorithmus, zusätzlich
    ...
    def _refresh_training_plot(self) -> None:
        ...
        if runtime.history:
            ...
            self.axes.plot(episodes, rolling_average(rewards, self.smoothing_window),
    ...
    def _refresh_comparison_plot(self) -> None:
        ...
        for slot in range(self.slot_count):
            ...
            self.comparison_axes.plot(x, y, color=SLOT_COLORS[slot], alpha=self.RAW_ALPHA,
                                      linewidth=.8)
            self.comparison_axes.plot(episodes,
                                      rolling_average(rewards, self.smoothing_window),
                                      color=SLOT_COLORS[slot],
                                      linestyle=SLOT_LINESTYLE, linewidth=self.LINE_WIDTH,
    ...
    @classmethod
    def _statistics(cls, history: list[EpisodeMetric], steps: int, budget: int,
                    generations: Optional[int] = None) -> tuple[Any, ...]:
        """`Generationen` besitzt nur CMA-ES; sonst steht dort `—`.

        Die Zeile macht sichtbar, wie wenige Update-Schritte hinter einer
        CMA-ES-Kurve stehen.
        """
        generation_text = "—" if generations is None else thousands(generations)
        if not history:
            return (0, thousands(steps), thousands(budget), generation_text, *(["—"] * 8))
        best = cls.best_episode(history)
        return (
            len(history), thousands(steps), thousands(budget), generation_text,
            ...
            german(float(np.mean([item.mean_speed for item in history])), 2),
            german(float(np.mean([item.distance for item in history])), 1),
            f"{np.mean([item.survived for item in history]):.1%}".replace(".", ","),
            f"{np.mean([item.fell for item in history]):.1%}".replace(".", ","),
            f"{np.mean([item.reached_target for item in history]):.1%}".replace(".", ","),
        )

    STAT_LABELS = ("Episoden", "Schritte", "Budget", "Generationen", "Ø Return",
                   "Beste Episode", "Ø Länge", "Ø Tempo m/s", "Ø Strecke m",
                   "Durchhaltequote", "Sturzquote", "Zielquote")
    ...
    def _evaluation_footer(self, prefix: str, result: EvaluationResult) -> str:
        return (f"{prefix}: Ø {german(result.mean_reward)} ± {german(result.reward_std)} | "
                ...
                f"durchgehalten {result.survive_rate:.0%} | Ziel {result.target_rate:.0%} | "
                f"v̄ₓ {german(result.mean_speed, 2)} m/s | "
                f"Strecke {german(result.mean_distance, 1)} m")

    def _training_summary(self) -> None:
        slot = self.active_slot
        runtime = self.slots[slot]
        model = runtime.workbench.model
        stats = self._statistics(runtime.history, runtime.workbench.num_timesteps,
                                 runtime.workbench.config.total_timesteps,
                                 runtime.workbench.generations)
        footer = ["Höhere Returns sind besser. Die Zielmarke "
                  f"{german(TARGET_RETURN, 0)} ist projektintern gesetzt – "
                  "Walker2d-v5 führt keinen offiziellen reward_threshold.",
                  f"Durchhalten = volle {MAX_EPISODE_STEPS} Schritte (truncated), "
                  "Sturz = ungesund geworden (terminated). Generationen nur bei CMA-ES."]
    ...
    def _comparison_summary(self) -> None:
        ...
        for slot in range(self.slot_count):
            ...
            statistics[SLOT_LABELS[slot]] = (config.algorithm, *self._statistics(
                runtime.comparison_history,
                runtime.comparison.num_timesteps if runtime.comparison else 0,
                config.total_timesteps,
                runtime.comparison.generations if runtime.comparison else None))
    ...
    def _difference_lines(self, configs: list[Walker2dConfig]) -> list[str]:
    ...
    def _export_base_name(self) -> str:
        ...
        return f"walker2d_{slug}_{self._export_stamp}"
    ...
    def _renderer(self, slot: int) -> Walker2dRenderer:
        ...
        if self.renderers[slot] is None:
            self.renderers[slot] = Walker2dRenderer(seed=self.slots[slot].workbench.config.seed)
    ...
    def _animation_workbench(self, slot: int) -> Any:
    ...
    def _hover_lines(self, slot: int) -> str:
        ...
        # Alle sechs Motoren haben dieselbe Übersetzung – anders als bei
        # HalfCheetah, wo sie sich unterscheiden.
        lines = [f"Action (Moment = aᵢ · gear {GEAR:.0f})"]
        ...
        if action is None:
            ...
            for index, joint in enumerate(action_readout(action)["joints"]):
                lines.append(f"  a{index} {joint['raw']:+.3f} {joint['joint']:<13}"
                             f"{joint['text']}")
        angles, speeds = values["joint_angles"], values["joint_velocities"]
        lines += [
            "",
            "Zustand (gesund: Höhe 0,80–2,00 · Winkel ±1,00)",
            ...
            "  Winkel      rechts " + " ".join(f"{value:+.2f}" for value in angles[:3]),
            "              links  " + " ".join(f"{value:+.2f}" for value in angles[3:]),
            f"  Tempo       vₓ {values['vx']:+.3f}  v_z {values['vz']:+.3f}  "
            f"ω {values['torso_angular_velocity']:+.3f}",
            "  Gelenktempo rechts " + " ".join(f"{value:+.2f}" for value in speeds[:3]),
            "              links  " + " ".join(f"{value:+.2f}" for value in speeds[3:]),
            "  Alle neun Tempi sind auf ±10 geclippt"
            ...
            f"  Steuerkosten {rewards['ctrl']:+.5f}",
    ...
    def _begin_episode(self, slot: int, live: bool) -> None:
        ...
        if best is not None:
            ...
            self.animation_snapshot[slot] = best[1]
            episode, seed = best[0].episode, configured_seed
        else:
            # Kopie des aktuellen Lernstands. Bei CMA-ES ist das der
            # Verteilungsmittelwert, nicht ein gezogener Kandidat.
            self.animation_snapshot[slot] = workbench.current_snapshot()
            episode = self.trained_episodes(slot)
            # Laufende Episoden starten ohne festen Seed, damit nacheinander
            # gezeigte Episoden nicht identisch aussehen.
            seed = None if live else configured_seed
    ...
    def _stop_live_animation(self) -> None:
        ...
        for slot in range(MAX_SLOTS):
            if self.animation_live[slot]:
                self.animation_live[slot] = False
                self.animation_snapshot[slot] = None
                self._stop_animation(slot)
        self._sync_panels()

    def _policy_action(self, slot: int) -> np.ndarray:
        """Action aus dem Lernstand, den diese Anzeige gerade spielt.

        Der Slot selbst rechnet: Bei den SB3-Verfahren ist der Lernstand ein
        Policy-State, bei CMA-ES ein Gewichtsvektor samt eingefrorener
        Beobachtungsstatistik. Die GUI muss den Unterschied nicht kennen.
        """
        return self._animation_workbench(slot).act(
            self.animation_snapshot[slot], self.animation_observation[slot])

    @staticmethod
    def _episode_outcome_text(terminated: bool) -> str:
        """Sturz heißt hier: ungesund geworden. Es gibt keine Terminalstrafe."""
        if terminated:
    ...
    def instructions(self) -> None:
        messagebox.showinfo(
            ...
            "Environment\n"
            "Walker2d-v5 (MuJoCo): a₀ bis a₅ sind Drehmomente für sechs Gelenke –\n"
            f"rechts und links je Hüfte, Knie und Fuß, alle mit derselben Übersetzung\n"
            f"gear = {GEAR:.0f}. Der Reward ist 1,0 fürs Überleben + 1,0·vₓ - 0,001·Σaᵢ².\n"
            "Es gibt weder Terminalbonus noch Terminalstrafe: Ein Sturz kostet nur die\n"
            "Rewards der Schritte, die nicht mehr stattfinden. Die Episode endet mit\n"
            "'terminated', sobald der Roboter ungesund wird – Höhe außerhalb 0,8 bis\n"
            "2,0 m (auch ein Sprung nach oben beendet sie!) oder Rumpfwinkel außerhalb\n"
            f"±1,0 rad –, und mit 'truncated' nach {MAX_EPISODE_STEPS} Schritten.\n"
            f"Walker2d-v5 führt KEINEN offiziellen reward_threshold. Die Zielmarke\n"
            f"{german(TARGET_RETURN, 0)} ist projektintern gesetzt, begründet durch die\n"
            "Zoo-Benchmarkwerte (SAC 3.863, TD3 4.718). Sie heißt deshalb Zielquote und\n"
            "nicht Gelöst-Quote.\n\n"
            "Methoden\n"
            "SAC und TD3 sind gradientenbasiert und off-policy, mit zwei Critics und\n"
            "Replay Buffer – TD3 mit deterministischem Actor und Action Noise, SAC mit\n"
            "stochastischem Actor und gelernter Entropie.\n"
            "CMA-ES ist etwas völlig anderes: gradientenfrei und populationsbasiert.\n"
            "Es zieht je Generation λ Gewichtsvektoren aus einer Normalverteilung,\n"
            "bewertet jeden über ganze Episoden und schätzt daraus Mittelwert,\n"
            "Schrittweite und Kovarianz neu. Es kennt weder Lernrate noch Batch-Größe\n"
            "noch Diskontfaktor und optimiert die UNDISKONTIERTE Rendite – also genau\n"
            "das, was der Graph zeigt. Nur die Rangfolge der Kandidaten zählt.\n"
            "Seine Policy ist bewusst winzig (linear, 108 Parameter): CMA-ES führt eine\n"
            "n×n-Kovarianzmatrix, bei 256,256 wären das über 40 GB.\n"
            "Bei gleichem Schrittbudget liegt CMA-ES deshalb typischerweise deutlich\n"
            "zurück – es nutzt je Episode nur eine einzige Zahl, während SAC und TD3\n"
            "jeden einzelnen Übergang auswerten. Das sagt nichts über die Qualität des\n"
            "Verfahrens, sondern über die Informationsmenge je Schritt. Die Zeile\n"
            "'Generationen' in der Summary macht sichtbar, wie wenige Update-Schritte\n"
            "hinter einer CMA-ES-Kurve stehen.\n"
            "Training exploriert, Evaluation ist deterministisch und ohne Lernupdates;\n"
            "bei CMA-ES heißt das: der Verteilungsmittelwert statt eines Kandidaten.\n\n"
            ...
            "die projektinterne Zielmarke; die Y-Achse folgt den Messwerten.\n"
            ...
            "besten Episode je Slot. 'Glättung' rechts in der Tableiste stellt ein,\n"
            "über wie viele Episoden der gleitende Durchschnitt mittelt – global für\n"
            "alle Slots und beide Graphen, sofort wirksam, auch mitten im Lauf. Bei 1\n"
            "fällt die Kurve mit den Rohwerten zusammen. Da jede Episode exakt\n"
            f"{MAX_EPISODE_STEPS} Schritte dauert, entspricht die Episodennummer auf der\n"
            "X-Achse genau dem tausendfachen Schrittfortschritt.\n"
            ...
            "Unter jedem Animationsbild stehen nur Episode, Schritt und Return; das\n"
            "Verfahren steht im Titel darüber. Alle weiteren Messwerte – Action mit\n"
            "gelenkeigener Übersetzung, Zustand, Reward-Zerlegung –\n"
            ...
            "Obergrenze und wird bei vier Anzeigen selten erreicht. Achtung: Bei der\n"
            f"environment-eigenen Rate von {RENDER_FPS} FPS dauert eine Episode 50\n"
            ...
            "Bei CMA-ES: zu kleines Budget für genügend Generationen, zu großes σ₀ oder\n"
            "abgeschaltete Beobachtungsnormalisierung – ohne sie arbeitet eine lineare\n"
            "Policy auf den 17 sehr unterschiedlich skalierten Werten kaum.\n"
            "Mit dem vollen Budget erreichen SAC und TD3 laut Zoo-Benchmark rund 3.863\n"
            "beziehungsweise 4.718. Für CMA-ES führt der Zoo keine Werte.",
            parent=self.root,
        )
