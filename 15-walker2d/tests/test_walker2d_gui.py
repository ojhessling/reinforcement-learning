"""Tests für Layout, Slotverwaltung, Farbgebung und Beschriftung der Walker2d-GUI."""
...
from walker2d_render import MUJOCO_BACKENDS, mujoco_gl_backend, register_backend


def metric(episode: int, reward: float, survived: bool = False,
           speed: float = 2.5) -> EpisodeMetric:
    length = MAX_EPISODE_STEPS if survived else 200
    return EpisodeMetric(episode, reward, length, survived, not survived,
                         reward >= TARGET_RETURN, episode * length,
                         speed, speed * length * 0.008)
...
class HelperTests(unittest.TestCase):
    ...
    def test_default_frame_rate_is_valid_and_matches_the_environment(self):
        self.assertEqual(RENDER_FPS, 125)
    ...
    def test_entry_point_exposes_main_without_starting_a_window(self):
        ...
        self.assertTrue(callable(walker2d_app.main))
        self.assertIs(walker2d_app.Walker2dGUI, Walker2dGUI)
...
class PaletteTests(unittest.TestCase):
    ...
    def test_line_widths_and_raw_transparency(self):
        self.assertEqual(Walker2dGUI.LINE_WIDTH, 1.2)
        self.assertAlmostEqual(Walker2dGUI.RAW_ALPHA, 0.10)
...
class GUITestCase(unittest.TestCase):
    def setUp(self):
        ...
        patcher = patch.object(Walker2dGUI, "_show_initial_frame", lambda self, slot=0: None)
        patcher.start()
        self.addCleanup(patcher.stop)
        self.app = Walker2dGUI(self.root)
        self.root.update()
...
class SlotCountTests(GUITestCase):
    def test_every_method_appears_exactly_once_by_default(self):
        self.assertEqual(DEFAULT_SLOT_COUNT, len(ALGORITHMS))
        ...
        # Jeder Algorithmus genau einmal – kein Slot ist doppelt belegt.
        self.assertEqual(set(algorithms), set(ALGORITHMS))
        self.assertEqual(len(algorithms), len(set(algorithms)))
        self.assertIn("CMA-ES", algorithms)

    def test_a_fourth_slot_doubles_a_method_and_gets_a_difference(self):
        """Erst ein vierter Slot doppelt zwangsläufig ein Verfahren."""
        self._set_slot_count(MAX_SLOTS)
        algorithms = [self.app.algorithm_vars[slot].get() for slot in range(MAX_SLOTS)]
        self.assertEqual(len(algorithms) - len(set(algorithms)), 1)
        repeated = [slot for slot in range(MAX_SLOTS)
                    if algorithms.count(algorithms[slot]) > 1]
        configs = [Walker2dGUI.initial_config(slot) for slot in repeated]
    ...
    def test_shrinking_without_data_needs_no_confirmation(self):
        with patch("walker2d_gui.messagebox.askyesno") as ask:
    ...
    def test_shrinking_asks_before_discarding_a_learning_state(self):
        ...
        with patch("walker2d_gui.messagebox.askyesno", return_value=False) as ask:
            self._set_slot_count(2)
            ask.assert_called_once()
        self.assertEqual(self.app.slot_count, DEFAULT_SLOT_COUNT)
        self.assertTrue(self.app.slots[2].history)
        with patch("walker2d_gui.messagebox.askyesno", return_value=True):
            self._set_slot_count(2)
        self.assertEqual(self.app.slot_count, 2)
...
class PlotTests(GUITestCase):
    def test_comparison_plot_draws_one_solid_line_per_slot_in_its_colour(self):
        ...
        highlighted = [line for line in self.app.comparison_axes.get_lines()
                       if line.get_linewidth() == Walker2dGUI.LINE_WIDTH
    ...
    def test_neither_graph_shows_the_deterministic_evaluation(self):
        ...
        self.app.slots[0].evaluations.append(
            (5, EvaluationResult(3, 900.0, 10.0, 1000.0, 1.0, 0.0, 0.0, 2.0, 16.0)))
    ...
    def test_raw_episodes_use_the_configured_transparency(self):
        ...
        for line in faint:
            self.assertAlmostEqual(line.get_alpha(), Walker2dGUI.RAW_ALPHA)

    def test_the_reference_line_is_white_dashed_and_not_called_solved(self):
        """`Walker2d-v5` hat keine offizielle Gelöst-Schwelle."""
        self.app._refresh_training_plot()
        self.app._refresh_comparison_plot()
        for axes in (self.app.axes, self.app.comparison_axes):
            ...
            labels = [str(line.get_label()) for line in axes.get_lines()]
            self.assertFalse([label for label in labels if "Gelöst" in label], labels)
...
class SummaryTests(GUITestCase):
    ...
    def test_comparison_summary_has_one_column_per_active_slot(self):
        ...
        for label in ("Durchhaltequote", "Sturzquote", "Zielquote", "Ø Länge",
                      "Ø Tempo m/s", "Ø Strecke m", "Generationen"):
            self.assertIn(label, text)
        # Ohne offizielle Schwelle heißt die Zeile nicht „Gelöst".
        self.assertNotIn("Gelöst-Quote", text)
    ...
    def test_summary_lists_the_best_episode_with_number_and_return(self):
        ...
        self.assertEqual(Walker2dGUI.best_episode(history).episode, 2)
        self.assertIsNone(Walker2dGUI.best_episode([]))
    ...
    def test_training_summary_reports_the_evaluation_metrics(self):
        result = EvaluationResult(5, 1234.5, 20.0, 1000.0, 0.8, 0.2, 0.0, 1.5, 12.0)
...
class CaptionAndHoverTests(GUITestCase):
    ...
    def test_hover_shows_actions_and_state_next_to_the_image(self):
        ...
        # Eine einheitliche Übersetzung für alle sechs Gelenke.
        self.assertIn(str(int(GEAR)), text)
        self.assertIn("±10", text)
        self.assertIn("Überleben", text)
        self.assertIn("ungesund", text)
...
class EpisodeChoiceTests(GUITestCase):
    ...
    def test_the_summary_works_with_a_real_learning_state(self):
        """Deckt die Slot-Schnittstelle ab, die beide Verfahrensklassen teilen."""
        for slot in range(self.app.slot_count):
            workbench = self.app.slots[slot].workbench
            for attribute in ("num_timesteps", "generations", "current_snapshot",
                              "best_snapshot", "act", "evaluate", "train", "save"):
                self.assertTrue(hasattr(workbench, attribute),
                                f"{workbench.config.algorithm} fehlt {attribute}")
            # Ohne Modell liefert der Zähler 0 statt zu krachen.
            self.assertEqual(workbench.num_timesteps, 0)
            self.app.slots[slot].comparison = workbench
            self.app.slots[slot].comparison_history.append(metric(1, 100.0))
        self.app._comparison_summary()
        self.assertIn("Generationen", self.app._summary_text())

    def test_generations_are_shown_only_for_cma_es(self):
        """Die Zeile macht sichtbar, wie wenige Updates hinter der Kurve stehen."""
        self._select(0)
        self.app.slots[0].history.append(metric(1, 100.0))
        self.app._training_summary()
        lines = [line for line in self.app._summary_text().splitlines()
                 if line.startswith("Generationen")]
        self.assertEqual(len(lines), 1)
        # TD3 kennt keine Generationen.
        self.assertIn("—", lines[0])
        self.assertEqual(self.app.slots[0].workbench.generations, None)
        cma = [slot for slot in range(self.app.slot_count)
               if self.app.algorithm_vars[slot].get() in EVOLUTIONARY_ALGORITHMS]
        self.assertTrue(cma)
        self.assertEqual(self.app.slots[cma[0]].workbench.generations, 0)
...
class ValidationTests(GUITestCase):
    ...
    def test_configuration_round_trips_through_the_entry_fields(self):
        for slot in range(self.app.slot_count):
            ...
            self.assertEqual(config, Walker2dGUI.initial_config(slot))
