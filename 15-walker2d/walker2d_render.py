...
def _worker(connection: Connection, seed: Optional[int]) -> None:
    ...
    try:
        ...
        from walker2d_logic import make_walker2d_env
...
class Walker2dRenderer:
    ...
    def _await_ready(self, timeout: float) -> bool:
        if self.ready:
            return True
        if self.closed:
            raise RendererUnavailable("Der isolierte Walker2d-Renderer ist bereits beendet.")
    ...
    def _exchange(self, message: tuple[str, object]):
        ...
        if self.closed or not self.process.is_alive():
            raise RendererUnavailable("Der isolierte Walker2d-Renderer ist nicht verfügbar.")
