"""Dark Tkinter GUI for the BipedalWalker PPO/TD3/SAC workbench."""
...
from bipedalwalker_logic import (
    ALGORITHMS, BOOLEAN_FIELDS, CHOICE_FIELDS, FALL_REWARD_THRESHOLD, INTEGER_FIELDS,
    OFF_POLICY_ALGORITHMS, OPTIONAL_FLOAT_FIELDS, SOLVED_RETURN, TUPLE_FIELDS,
    EpisodeMetric, EvaluationResult, BipedalWalkerConfig, BipedalWalkerWorkbench,
...
class SlotRuntime:
    ...
    def __init__(self, config: BipedalWalkerConfig, best_base: Path) -> None:
        self.workbench = BipedalWalkerWorkbench(config)
        self.comparison: Optional[BipedalWalkerWorkbench] = None
...
class BipedalWalkerGUI:
    ...
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.renderers: list[Optional[BipedalWalkerRenderer]] = [BipedalWalkerRenderer(), None]
        ...
        self.checkpoints = tempfile.TemporaryDirectory(prefix="bipedalwalker-")
    ...
    def _variables(self) -> None:
        ...
        for slot, name in enumerate(DEFAULT_SLOT_ALGORITHMS):
            ...
            for field in fields(BipedalWalkerConfig):
    ...
    def _fill_values(self, slot: int, config: BipedalWalkerConfig) -> None:
    ...
    def _window(self) -> None:
        ...
        self.root.title("BipedalWalker Workbench")
    ...
    def _layout(self) -> None:
        ...
        ttk.Label(header, text="BipedalWalker Workbench",
                  font=("TkDefaultFont", 18, "bold")).pack(side="left")
        ttk.Label(header, text="PPO, TD3 und SAC – vier Gelenke, ein Ziel: laufen").pack(side="left", padx=16)
    ...
    def _algorithm_changed(self, slot: int) -> None:
        ...
        self._build_tab(slot)
        runtime.workbench.close(); runtime.workbench = BipedalWalkerWorkbench(config)
    ...
    def _config(self, slot: int) -> BipedalWalkerConfig:
        data: dict[str, Any] = {}
        for name, variable in self.values[slot].items():
            if name in BOOLEAN_FIELDS:
        ...
        config = BipedalWalkerConfig(algorithm=self.algorithm_vars[slot].get(), **data)
        try:
            config.validate()
        except ValueError as error:
    ...
    def start_comparison(self) -> None:
        ...
        for slot, config in enumerate(configs):
            ...
            if runtime.comparison is None:
                runtime.comparison = BipedalWalkerWorkbench(config)
            else:
    ...
    def restore_best(self) -> None:
        ...
        try:
            restored = BipedalWalkerWorkbench.load(runtime.best_base, runtime.algorithm)
        except Exception as error:
    ...
    def _series_label(self, slot: int, configs: Optional[list[BipedalWalkerConfig]] = None) -> str:
    ...
    def _refresh_training_plot(self) -> None:
        ...
        self.axes.axhline(SOLVED_RETURN, color="#a3e635", linestyle=":", linewidth=1.2, label="Gelöst ab +300")
    ...
    def _statistics(history: list[EpisodeMetric], steps: int, budget: int) -> tuple[Any, ...]:
        ...
        return (
            ...
            f"{np.mean([item.finished for item in history]):.1%}",
            f"{np.mean([item.solved for item in history]):.1%}",
            f"{np.mean([item.fell for item in history]):.1%}",
            f"{np.mean([item.timeout for item in history]):.1%}",
        )
    ...
    def _training_summary(self) -> None:
        ...
        labels = ("Episoden", "Schritte", "Budget", "Ø Return", "Zielquote", "Gelöst-Quote",
                  "Sturzquote", "Zeitlimitquote")
        ...
        if runtime.evaluations:
            ...
            footer.append(
                ...
                f"Ziel {last.goal_rate:.0%} | gelöst {last.solved_rate:.0%} | "
                f"Länge {last.mean_length:.0f}"
            )
        if runtime.best is not None:
            best = runtime.best
            footer.append(
                f"Beste Evaluation: Ø {best.mean_reward:.1f} | Ziel {best.goal_rate:.0%} | "
    ...
    def _comparison_summary(self) -> None:
        ...
        if not any(runtime.comparison_history for runtime in self.slots):
            statistics = {}
        labels = ("Algorithmus", "Episoden", "Schritte", "Budget", "Ø Return", "Zielquote",
                  "Gelöst-Quote", "Sturzquote", "Zeitlimitquote")
        footer = []
        for slot, runtime in enumerate(self.slots):
    ...
    def _difference_lines(self, configs: list[BipedalWalkerConfig]) -> list[str]:
    ...
    def _export_base_name(self) -> str:
        ...
        return f"bipedalwalker_{slug}_{self._export_stamp}"
    ...
    def _renderer(self, slot: int) -> BipedalWalkerRenderer:
        ...
        if all(self.animation_live):
            self.renderers[slot] = BipedalWalkerRenderer()
            return self.renderers[slot]
    ...
    def _animation_workbench(self, slot: int) -> BipedalWalkerWorkbench:
    ...
    def _show_readout(self, slot: int, observation: np.ndarray,
        ...
        """Alle 24 Beobachtungswerte und die vier Drehmomente ausweisen.

        Geschwindigkeiten und Gelenkwerte sind im Environment bereits skaliert
        und werden deshalb als normierte Größen beschriftet; nur der Rumpfwinkel
        ist ein echter Winkel und erscheint zusätzlich in Grad.
        """
        values = observation_readout(observation)
        lines = [
            f"Schritt: {self.animation_step[slot]:4d}   "
        ...
        if action is None:
            ...
            raw = " ".join(f"a{index}={value:+.2f}" for index, value in enumerate(readout["raw"]))
            lines.append(f"Action: {raw}")
            lines.append("Moment: " + "   ".join(
                f"{joint['joint']}: {joint['text']}" for joint in readout["joints"]))
        lines.append(
            f"Rumpf: {values['hull_angle']:+.3f} rad ({values['hull_angle_degrees']:+5.1f}°)   "
            f"ω*: {values['hull_angular_velocity']:+.3f}   "
            f"vx*: {values['vx']:+.3f}   vy*: {values['vy']:+.3f}")
        for index, leg in enumerate(values["legs"], start=1):
            lines.append(
                f"Bein {index}: Hüfte {leg['hip_angle']:+.2f} / {leg['hip_speed']:+.2f}*   "
                f"Knie {leg['knee_angle']:+.2f} / {leg['knee_speed']:+.2f}*   "
                f"Boden: {'ja' if leg['contact'] else 'nein'}")
        lines.append("Lidar: " + " ".join(f"{value:.2f}" for value in values["lidar"]))
        lines.append("* normierte Werte · Lidar: kleiner = näherer Boden")
    ...
    def _episode_outcome_text(self, reward: float, truncated: bool) -> str:
        ...
        if reward <= FALL_REWARD_THRESHOLD:
            return "Abgeschlossen – gestürzt"
        return "Abgeschlossen – Strecke geschafft"
    ...
    def instructions(self) -> None:
        messagebox.showinfo(
            ...
            "Environment\n"
            "BipedalWalker-v3 kontinuierlich: a₀ regelt das Haupttriebwerk (≤ 0 aus, sonst\n"
