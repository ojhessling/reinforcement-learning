...
from humanoid_extended_gui import (
    BEST_EPISODE,
    CURRENT_EPISODE,
...
from humanoid_extended_render import MUJOCO_BACKENDS, mujoco_gl_backend, register_backend
...
class HelperTests(unittest.TestCase):
    ...
    def test_entry_point_exposes_main_without_starting_a_window(self):
        import humanoid_extended_app

        self.assertTrue(callable(humanoid_extended_app.main))
        self.assertIs(humanoid_extended_app.HumanoidGUI, HumanoidGUI)
...
class SlotCountTests(GUITestCase):
    def test_three_slots_are_the_default_one_per_compared_method(self):
        """Die Startbelegung ist der Vergleich dieser Erweiterung: SAC als
        Referenz, CrossQ und TQC. PPO und TD3 bleiben waehlbar, gehoeren aber
        nicht zum Vergleich - sie stehen im Projekt."""
        ...
        self.assertEqual(algorithms, ["SAC", "CrossQ", "TQC"])
        self.assertTrue(set(algorithms) <= set(ALGORITHMS))
    ...
    def test_shrinking_without_data_needs_no_confirmation(self):
        with patch("humanoid_extended_gui.messagebox.askyesno") as ask:
    ...
    def test_shrinking_asks_before_discarding_a_learning_state(self):
        ...
        with patch("humanoid_extended_gui.messagebox.askyesno", return_value=False) as ask:
            self._set_slot_count(2)
            ask.assert_called_once()
        self.assertEqual(self.app.slot_count, DEFAULT_SLOT_COUNT)
        self.assertTrue(self.app.slots[2].history)
        with patch("humanoid_extended_gui.messagebox.askyesno", return_value=True):
            self._set_slot_count(2)
        self.assertEqual(self.app.slot_count, 2)
...
class LongRunHintTests(GUITestCase):
    ...
    def test_the_default_budget_starts_without_a_question(self):
        ...
        with patch("humanoid_extended_gui.messagebox.askyesno") as frage:
    ...
    def test_a_long_run_names_its_duration(self):
        ...
        konfigurationen = [self.app._config(0)]
        with patch("humanoid_extended_gui.messagebox.askyesno", return_value=False) as frage:
    ...
    def test_several_slots_are_named_as_taking_longer(self):
        konfigurationen = [dataclasses.replace(self.app._config(0), total_timesteps=500_000)
                           for _ in range(3)]
        with patch("humanoid_extended_gui.messagebox.askyesno", return_value=True) as frage:
            self.assertTrue(self.app._confirm_long_run(konfigurationen))
