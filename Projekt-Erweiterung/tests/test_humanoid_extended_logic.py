...
from humanoid_extended_logic import (
    ACTION_DIM,
    ACTION_LIMIT,
    ACTION_NOISES,
    ACTUATOR_GROUPS,
...
class NeueVerfahrenTests(unittest.TestCase):
    """CrossQ und TQC - was diese Erweiterung gegenueber dem Projekt ergaenzt."""

    def test_crossq_has_no_target_network(self):
        """Kern des Verfahrens: Statt eines nachgezogenen Zielnetzes laufen
        beide Zustaende gemeinsam durch den Critic. Wo kein Target-Netz ist,
        darf auch kein `tau` uebergeben werden - der Konstruktor kennt es
        nicht und wuerde den Lauf abbrechen."""
        config = default_config("CrossQ")
        config.validate()
        kwargs = config.model_kwargs()
        self.assertNotIn("tau", kwargs)
        self.assertNotIn("target_update_interval", kwargs)
        self.assertFalse(config.uses("tau"))
        self.assertFalse(config.uses("target_update_interval"))
        self.assertNotIn("CrossQ", TARGET_NETWORK_ALGORITHMS)

    def test_crossq_really_builds_without_a_target_critic(self):
        """Nicht nur die Argumente - das erzeugte Modell selbst."""
        config = dataclasses.replace(tiny_config("CrossQ"), total_timesteps=200)
        workbench = HumanoidWorkbench(config)
        try:
            workbench.create_model()
            self.assertIsNone(getattr(workbench.model.policy, "critic_target", None))
        finally:
            workbench.close()

    def test_crossq_keeps_batch_normalization_and_its_wide_critic(self):
        config = default_config("CrossQ")
        policy = config.policy_kwargs()
        self.assertTrue(policy["batch_norm"])
        self.assertEqual(policy["batch_norm_momentum"], 0.01)
        self.assertEqual(policy["renorm_warmup_steps"], 100_000)
        self.assertEqual(tuple(policy["net_arch"]["qf"]), CROSSQ_CRITIC_ARCH)
        self.assertEqual(tuple(policy["net_arch"]["pi"]), DEFAULT_NET_ARCH)

    def test_crossq_rejects_an_impossible_batch_norm_momentum(self):
        for wert in (0.0, 1.0, -0.1, 1.5):
            with self.subTest(momentum=wert):
                config = dataclasses.replace(default_config("CrossQ"), batch_norm_momentum=wert)
                with self.assertRaises(ValueError) as fehler:
                    config.validate()
                self.assertIn("BatchNorm-Momentum", str(fehler.exception))

    def test_tqc_passes_its_quantiles_where_they_belong(self):
        """`n_quantiles` gehoert in die Policy (der Critic baut die Koepfe),
        `top_quantiles_to_drop_per_net` in den Algorithmus (er wirft sie beim
        Lernen weg)."""
        config = default_config("TQC")
        config.validate()
        self.assertEqual(config.policy_kwargs()["n_quantiles"], 25)
        self.assertEqual(config.model_kwargs()["top_quantiles_to_drop_per_net"], 2)
        self.assertIn("tau", config.model_kwargs())

    def test_tqc_never_drops_all_quantiles(self):
        """Wuerden alle Quantile verworfen, haette der Critic nichts mehr zu
        schaetzen."""
        config = dataclasses.replace(default_config("TQC"), n_quantiles=5,
                                     top_quantiles_to_drop=5)
        with self.assertRaises(ValueError) as fehler:
            config.validate()
        self.assertIn("Verworfene Quantile k", str(fehler.exception))
        erlaubt = dataclasses.replace(config, top_quantiles_to_drop=4)
        erlaubt.validate()
        keins = dataclasses.replace(config, top_quantiles_to_drop=0)
        keins.validate()

    def test_the_three_compared_methods_share_every_budget_parameter(self):
        """Verglichen wird der Critic, nicht das Budget: Schritte, Lernstart,
        Batch, Buffer und Diskont sind bei allen dreien gleich."""
        konfigurationen = [default_config(name) for name in ("SAC", "CrossQ", "TQC")]
        for feld in ("total_timesteps", "learning_starts", "batch_size",
                     "buffer_size", "gamma"):
            werte = {getattr(config, feld) for config in konfigurationen}
            self.assertEqual(len(werte), 1, f"{feld}: {werte}")

    def test_all_three_use_automatic_entropy_like_sac(self):
        for name in ("SAC", "CrossQ", "TQC"):
            with self.subTest(algorithmus=name):
                config = default_config(name)
                self.assertEqual(config.sac_ent_coef(), "auto_1.0")
                self.assertEqual(config.sac_target_entropy(), "auto")
                self.assertIn(name, ENTROPY_ALGORITHMS)
...
class ConfigurationTests(unittest.TestCase):
    def test_every_algorithm_starts_with_the_same_step_budget(self):
        ...
        # Nur fuer die Zoo-Verfahren ist ein Profilbudget hinterlegt; CrossQ und
        # TQC laufen mit den Voreinstellungen von sb3-contrib, fuer die es keins
        # gibt. Ein erfundener Wert waere eine Behauptung.
        self.assertEqual(set(PROFILE_BUDGET), {"PPO", "TD3", "SAC"})
        for name, budget in PROFILE_BUDGET.items():
            self.assertLess(REPORT_TIMESTEPS, budget, name)
    ...
    def test_only_crossq_keeps_its_own_critic_width(self):
        """Im Projekt wurden alle Netze vereinheitlicht, damit der Vergleich
        nicht an der Groesse haengt. Hier gilt das Gegenteil: CrossQs breiter
        Critic IST das Verfahren - Batch Normalization erlaubt ihn, und ohne
        ihn vergleicht man nicht mehr CrossQ."""
        for name in ALGORITHMS:
            config = default_config(name)
            self.assertEqual(config.actor_arch, DEFAULT_NET_ARCH, name)
            erwartet = CROSSQ_CRITIC_ARCH if name == "CrossQ" else DEFAULT_NET_ARCH
            self.assertEqual(config.critic_arch, erwartet, name)
        self.assertEqual(CROSSQ_CRITIC_ARCH, (1024, 1024))
