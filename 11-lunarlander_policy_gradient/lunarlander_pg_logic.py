"""SB3 learning logic for the continuous LunarLander policy-gradient workbench."""
...
#: Dieses Projekt nutzt zwingend die kontinuierliche Variante, weil TD3 und SAC
#: ausschließlich mit kontinuierlichen Actions arbeiten.
CONTINUOUS = True
...
#: Box(-1, 1, (2,)): Haupttriebwerk und Steuertriebwerke.
ACTION_DIM = 2
#: Ab diesem Betrag zündet ein Steuertriebwerk (Gymnasium-Spezifikation).
...
BOOLEAN_FIELDS = frozenset({"normalize_advantage", "use_sde"})
...
#: lohnen sich erst bei entsprechend großem Budget. Ein gemeinsamer Startwert
#: hält zudem den Vergleich zweier Verfahren an dieser Stelle gleich.
DEFAULT_NET_ARCH = (64, 64)

#: Einheitliches Trainingsbudget aller Verfahren. Die Zoo-Profile nennen sehr
#: unterschiedliche Budgets (PPO 1.000.000, TD3 300.000, SAC 500.000 Schritte);
#: für eine interaktive Workbench wäre das weder abwartbar noch vergleichbar.
#: Ein gemeinsamer Startwert macht den Vergleich zweier Slots ohne Zutun fair.
DEFAULT_TOTAL_TIMESTEPS = 100_000

#: Getunte `LunarLanderContinuous-v3`-Profile des RL Baselines3 Zoo. Werte, die
#: dort nicht auftauchen, bleiben auf den Voreinstellungen von
#: Stable-Baselines3 (siehe Dataclass-Defaults).
DEFAULT_PROFILES: dict[str, dict[str, Any]] = {
    "PPO": {
        "total_timesteps": DEFAULT_TOTAL_TIMESTEPS, "learning_rate": 3e-4, "batch_size": 64,
        "gamma": 0.999, "n_steps": 1024, "n_epochs": 4, "gae_lambda": 0.98, "ent_coef": 0.01,
        "actor_arch": DEFAULT_NET_ARCH, "critic_arch": DEFAULT_NET_ARCH, "activation": "Tanh",
        "optimizer_eps": 1e-5,
    },
    "TD3": {
        "total_timesteps": DEFAULT_TOTAL_TIMESTEPS, "learning_rate": 1e-3, "gamma": 0.98, "batch_size": 256,
        "buffer_size": 200_000, "learning_starts": 10_000, "train_freq": 1, "gradient_steps": 1,
    ...
    "SAC": {
        ...
        "learning_rate_schedule": "linear fallend", "batch_size": 256, "gamma": 0.99,
        "tau": 0.01, "buffer_size": 1_000_000, "learning_starts": 10_000,
...
def make_lunarlander_env(render_mode: Optional[str] = "rgb_array") -> gymnasium.Env:
    """Offizielles Environment; `continuous=True` ist für TD3/SAC zwingend."""
    kwargs = {} if render_mode is None else {"render_mode": render_mode}
    return gymnasium.make(ENV_ID, continuous=CONTINUOUS, **kwargs)
...
class LunarLanderPGConfig:
    ...
    @classmethod
    def from_dict(cls, values: dict[str, Any]) -> "LunarLanderPGConfig":
        data = dict(values)
...
def default_config(algorithm: str) -> LunarLanderPGConfig:
    ...
    return LunarLanderPGConfig(algorithm=algorithm, **DEFAULT_PROFILES[algorithm])
...
def config_differences(
    first: LunarLanderPGConfig, second: LunarLanderPGConfig
...
def action_readout(action: np.ndarray) -> dict[str, Any]:
    """Kontinuierliche Action in ihre Triebwerksbedeutung übersetzen.

    Gymnasium-Spezifikation: `a₀ ≤ 0` schaltet das Haupttriebwerk aus, `a₀ > 0`
    regelt den Schub linear von 50 % bis 100 %. `a₁ < -0,5` zündet das linke,
    `a₁ > 0,5` das rechte Steuertriebwerk, ebenfalls mit 50 % bis 100 %.
    """
    main_raw = float(np.clip(np.asarray(action).reshape(-1)[0], -1.0, 1.0))
    lateral_raw = float(np.clip(np.asarray(action).reshape(-1)[1], -1.0, 1.0))
    main_power = (main_raw + 1.0) * 0.5 if main_raw > 0.0 else 0.0
    if abs(lateral_raw) > LATERAL_THRESHOLD:
        side = "links" if lateral_raw < 0 else "rechts"
        lateral_power = (abs(lateral_raw) + 1.0) * 0.5
    else:
        side, lateral_power = "aus", 0.0
    return {
        "main_raw": main_raw, "lateral_raw": lateral_raw,
        "main_power": main_power, "lateral_side": side, "lateral_power": lateral_power,
        "main_text": "aus" if main_power == 0.0 else f"{main_power:.0%}",
        "lateral_text": "aus" if side == "aus" else f"{side} {lateral_power:.0%}",
    }


class LunarLanderPGCallback(BaseCallback):
...
class LunarLanderPGWorkbench:
    """Ein Verfahrensslot: Environment, Modell, Historie, Speichern und Laden."""

    def __init__(self, config: Optional[LunarLanderPGConfig] = None) -> None:
    ...
    def train(
        ...
        if reset:
            self.create_model()
        callback = LunarLanderPGCallback(
    ...
    def save(self, path: str | Path) -> tuple[Path, Optional[Path], Path]:
        ...
        metadata = {
            "format": 1, "environment": ENV_ID, "continuous": CONTINUOUS,
    ...
    def load(
        ...
        if not metadata_path.is_file():
            raise ValueError(
                ...
                "Ein Speicherstand besteht aus Modell, Metadaten und – bei TD3 und "
                "SAC – dem Replay Buffer."
            )
        metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
        if metadata.get("format") != 1 or metadata.get("environment") != ENV_ID \
                or metadata.get("continuous") != CONTINUOUS:
            raise ValueError(
                f"Inkompatibler Checkpoint: erwartet Format 1 für {ENV_ID} "
                f"(continuous={CONTINUOUS}), gefunden Format {metadata.get('format')} für "
                f"{metadata.get('environment')} (continuous={metadata.get('continuous')})."
            )
        algorithm = metadata.get("algorithm")
        if expected_algorithm is not None and algorithm != expected_algorithm:
            raise ValueError(
                f"Verfahren passt nicht: Die Datei enthält '{algorithm}', der aktive Slot "
                f"verwendet '{expected_algorithm}'. Wähle im Dropdown '{algorithm}' oder "
                "eine andere Datei."
            )
        workbench = cls(LunarLanderPGConfig.from_dict(metadata["config"]))
        workbench.env = make_lunarlander_env()
