...
from lunarlander_pg_gui import (
    EXPORT_DIR,
    MAX_ANIMATION_FPS,
    MIN_ANIMATION_FPS,
    PARAMETER_GROUPS,
...
from lunarlander_pg_logic import ALGORITHMS, OFF_POLICY_ALGORITHMS, default_config
...
class PlotHelperTests(unittest.TestCase):
    ...
    def test_entry_point_exposes_main_without_starting_a_window(self):
        import lunarlander_pg_app

        self.assertTrue(callable(lunarlander_pg_app.main))
        self.assertIs(lunarlander_pg_app.LunarLanderPGGUI, LunarLanderPGGUI)
...
class GUITestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.app = LunarLanderPGGUI(self.root)
        self.root.update()
...
class ControlTests(GUITestCase):
    ...
    def test_invalid_frame_rate_blocks_a_run_with_a_clear_message(self):
        self.app.fps.set("0")
        with patch("lunarlander_pg_gui.messagebox.showerror") as dialog:
    ...
    def test_differing_budgets_are_flagged_before_the_comparison_starts(self):
        ...
        with patch("lunarlander_pg_gui.messagebox.askyesno", return_value=False) as dialog:
...
class ExportTests(GUITestCase):
    def test_export_chart_writes_png_of_the_selected_tab(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("lunarlander_pg_gui.filedialog.asksaveasfilename", return_value=path):
    ...
    def test_exports_default_into_the_project_exports_folder(self):
        with tempfile.TemporaryDirectory() as directory:
            with patch("lunarlander_pg_gui.filedialog.asksaveasfilename",
    ...
    def test_chart_and_summary_share_a_matching_base_name(self):
        ...
        with patch("lunarlander_pg_gui.filedialog.asksaveasfilename", side_effect=fake_dialog):
            self.app.export_chart()
            self.app.export_summary()
...
class ComparisonRunTests(GUITestCase):
    ...
    def _configure(self, slot: int, **values) -> None:
        self._set_algorithm(slot, "TD3")
        # 600 Schritte reichen sicher für mehrere vollständige Episoden; eine
        # Episode endet erst mit Landung, Absturz oder dem Limit von 1000.
        defaults = dict(total_timesteps=600, learning_starts=16, train_freq=32, batch_size=16,
    ...
    def test_live_animation_shows_both_verfahren_during_a_comparison(self):
        self._configure(0, total_timesteps=3000)
        self._configure(1, total_timesteps=3000)
        self.app.evaluation_interval.set("0")
    ...
    def test_animation_can_be_switched_on_and_off_during_a_run(self):
        self._configure(0, total_timesteps=2000)
        self._configure(1, total_timesteps=2000)
        self.app.evaluation_interval.set("0")
        self.app.animation_enabled.set(False)
        self.app.start_comparison()
        self.root.update()
...
def _metric():
    from lunarlander_pg_logic import EpisodeMetric
    return EpisodeMetric(1, -120.0, 80, False, True, False, 80)
