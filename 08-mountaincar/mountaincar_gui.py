"""Dark Tkinter GUI for the MountainCar DDQN workbench."""
...
from mountaincar_logic import (
    ACTIVATIONS, ALGORITHMS, OPTIMIZERS, EpisodeMetric, EvaluationResult,
    MountainCarConfig, MountainCarWorkbench,
)
...
class MountainCarGUI:
    ...
    def __init__(self, root: tk.Tk) -> None:
        self.root = root
        self.workbench = MountainCarWorkbench()
        self.events: queue.Queue = queue.Queue()
        self.stop_event = threading.Event()
        self.worker: Optional[threading.Thread] = None
        self.busy = False
        self.renderer = MountainCarRenderer()
        ...
        self.comparison_workbenches: dict[str, MountainCarWorkbench] = {}
        self.active_algorithms: tuple[str, ...] = ()
        self.evaluations: list[EvaluationResult] = []
        self.best_evaluation: Optional[EvaluationResult] = None
        self.best_checkpoint = tempfile.TemporaryDirectory(prefix="mountaincar-best-")
    ...
    def _variables(self) -> None:
        ...
        self.observation = tk.StringVar(value="Position: —   Geschwindigkeit: —")

    def _window(self) -> None:
        width = min(1500, max(1200, self.root.winfo_screenwidth() - 40))
        height = min(920, max(760, self.root.winfo_screenheight() - 70))
        self.root.title("MountainCar DDQN Workbench")
    ...
    def _layout(self) -> None:
        ...
        ttk.Label(header, text="MountainCar DDQN Workbench", font=("TkDefaultFont", 18, "bold")).pack(side="left")
        ttk.Label(header, text="Vier Double-DQN-Erweiterungen – Zielposition 0,5 erreichen").pack(side="left", padx=16)
        ...
        upper.columnconfigure(0, minsize=740); upper.columnconfigure(1, weight=1); upper.rowconfigure(0, weight=1)
        self.controls = ttk.Frame(upper, width=740, padding=(0, 0, 8, 0))
        ...
        lower.columnconfigure(0, weight=3); lower.columnconfigure(1, weight=1, minsize=500); lower.rowconfigure(0, weight=1)
        ...
        self.figure.subplots_adjust(left=.10, right=.80, bottom=.24, top=.88)
        self.axes = self.figure.add_subplot(111)
        self.canvas = FigureCanvasTkAgg(self.figure, master=chart)
        self.canvas.get_tk_widget().pack(fill="both", expand=True)
        ttk.Label(info, textvariable=self.summary, justify="left", font="TkFixedFont").pack(anchor="nw")
    ...
    def _controls(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(training, text="Algorithmus").grid(row=0, column=0, sticky="w")
        combo = ttk.Combobox(training, textvariable=self.algorithm, values=ALGORITHMS, state="readonly", width=15)
        combo.grid(row=0, column=1, sticky="ew"); combo.bind("<<ComboboxSelected>>", lambda _event: self._algorithm_fields())
        ...
        self.variant_entries: dict[str, list[ttk.Entry]] = {name: [] for name in ALGORITHMS}
        specifications = (
            ("Noisy DDQN", "Noisy σ₀", "noisy_sigma_0"),
            ("PER DDQN", "PER α", "per_alpha"), ("PER DDQN", "PER β₀", "per_beta_0"),
            ("PER DDQN", "β-Annealing", "per_beta_steps"), ("PER DDQN", "PER ε", "per_epsilon"),
            ("Dueling DDQN", "Value-Stream", "value_arch"),
            ("Dueling DDQN", "Advantage-Stream", "advantage_arch"),
        )
        compact_labels = {
            "Noisy σ₀": "Noisy σ₀", "PER α": "PER α", "PER β₀": "PER β₀",
            "β-Annealing": "β-Anneal.", "PER ε": "PER ε",
            "Value-Stream": "Value hᵥ", "Advantage-Stream": "Advant. hₐ",
        }
        for index, (algorithm, label, name) in enumerate(specifications):
            row, group = divmod(index, 2); column = group * 2
            ttk.Label(variants_more, text=compact_labels[label]).grid(
                row=row, column=column, sticky="w", padx=(0, 3), pady=1
            )
            entry = ttk.Entry(variants_more, textvariable=self.values[name], width=7)
            entry.grid(row=row, column=column + 1, sticky="ew", padx=(0, 5), pady=1)
            self.variant_entries[algorithm].append(entry)
    ...
    def _algorithm_fields(self) -> None:
        ...
        for algorithm, entries in self.variant_entries.items():
            for entry in entries:
                entry.configure(state="normal" if algorithm == selected else "disabled")
        state = "disabled" if selected == "Noisy DDQN" else "normal"
    ...
    def _config(self) -> MountainCarConfig:
        try:
            data: dict[str, Any] = {}
            integers = {"total_timesteps", "buffer_size", "learning_starts", "batch_size", "train_freq", "gradient_steps", "target_update_interval", "per_beta_steps"}
            ...
            config = MountainCarConfig(algorithm=self.algorithm.get(), **data)
    ...
    def start_training(self) -> None:
        ...
        if self.workbench.model is not None and config.signature() != self.workbench.config.signature():
            ...
            self.workbench.close(); self.workbench = MountainCarWorkbench(config); self.evaluations.clear(); self.best_evaluation = None
    ...
    def start_comparison(self) -> None:
        ...
        for name in algorithms:
            ...
            self.comparison_workbenches.setdefault(name, MountainCarWorkbench(replace(config, algorithm=name)))
    ...
    def restore_best(self) -> None:
        ...
        try: restored = MountainCarWorkbench.load(self.best_base)
    ...
    def _refresh_training_plot(self) -> None:
        ...
        self.axes.axhline(-110, color="#fbbf24", linestyle="--", linewidth=1.1, label="Referenz -110")
    ...
    def _refresh_comparison_plot(self) -> None:
        self.axes.clear(); colors = {name: color for name, color in zip(ALGORITHMS, ("#60a5fa", "#34d399", "#f87171", "#c084fc"))}
    ...
    def _table(self, title: str, statistics: dict[str, tuple[Any, ...]]) -> None:
        ...
        algorithms = list(statistics); width, value_width = 11, 12
    ...
    def _show_initial_frame(self) -> None:
        ...
        self.observation.set(f"Position: {observation[0]:+.4f}   Geschwindigkeit: {observation[1]:+.4f}")
    ...
    def _animate_step(self) -> None:
        ...
        action_text = ("links", "keine Beschleunigung", "rechts")[int(action)]
        self.observation.set(f"Schritt: {self.animation_step}   Action: {action_text}   Reward: {self.animation_reward:.0f}\nPosition: {observation[0]:+.4f}   Geschwindigkeit: {observation[1]:+.4f}   Ziel: 0,5")
    ...
    def reset(self) -> None:
        ...
        self.workbench.close(); self.workbench = MountainCarWorkbench(config); self.live_history.clear(); self.evaluations.clear(); self.best_evaluation = None
    ...
    def load(self) -> None:
        ...
        if not path: return
        try: loaded = MountainCarWorkbench.load(path)
    ...
    def instructions(self) -> None:
        messagebox.showinfo("Bedienungsanleitung", "1. Algorithmus und Parameter wählen.\n2. Trainieren oder mehrere Varianten vergleichen.\n3. Reward -1 je Schritt: weniger negativ ist besser.\n4. Ziel ist Position 0,5 vor Schritt 200.\n5. Evaluation lernt nicht und NoisyNet verwendet dabei kein Rauschen.\n6. Bestes Modell kann vollständig wiederhergestellt werden.", parent=self.root)
