    ...
    try:
        while True:
            ...
            if command == "reset":
                ...
                observation, reward, terminated, truncated, info = env.step(int(payload))
...
class LunarLanderRenderer:
    ...
    def step(self, action: int) -> tuple[np.ndarray, float, bool, bool, dict, np.ndarray]:
        ...
        return np.asarray(observation), float(reward), bool(terminated), bool(truncated), dict(info), np.asarray(frame)

    def _check(self) -> None:
        if self.closed or not self.process.is_alive():
            raise RuntimeError("Der isolierte LunarLander-Renderer ist nicht verfügbar.")
