...
def _worker(connection: Connection, seed: Optional[int]) -> None:
    ...
    try:
        ...
        from hopper_logic import make_hopper_env
...
class HopperRenderer:
    ...
    def _await_ready(self, timeout: float) -> bool:
        if self.ready:
            return True
        if self.closed:
            raise RendererUnavailable("Der isolierte Hopper-Renderer ist bereits beendet.")
    ...
    def _exchange(self, message: tuple[str, object]):
        ...
        if self.closed or not self.process.is_alive():
            raise RendererUnavailable("Der isolierte Hopper-Renderer ist nicht verfügbar.")
